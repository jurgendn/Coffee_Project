...
class User(UserMixin, db.Model):
    ...
    link_img=db.Column(db.String(100))
...
def home():
    ...
    if check is None:
        new_count=Count_Visit(date_visit=time,web_home=1,web_about=0,web_store=0,web_booking=0,web_coffeesraw=0,web_machinery=0)
...
def about():
    ...
    check=Count_Visit.query.filter_by(date_visit=time).first()
    if check is None:
        new_count=Count_Visit(date_visit=time,web_home=0,web_about=1,web_store=0,web_booking=0,web_coffeesraw=0,web_machinery=0)
        db.session.add(new_count)
        db.session.commit()
    else:
...
def coffees():
    ...
    check=Count_Visit.query.filter_by(date_visit=time).first()
    if check is None:
        new_count=Count_Visit(date_visit=time,web_home=0,web_about=0,web_store=0,web_booking=0,web_coffeesraw=1,web_machinery=0)
        db.session.add(new_count)
        db.session.commit()
    else:
...
def search():
    if 1>0:
        ...
        if len(finish_product_sure)==0:
            ...
            flash('Category: '+search_category+'--- Price: '+search_price+'--- More feature: '+input_search)
...
def cart():
    ...
    for i in range(len(all_receipt)):
        ...
        lst_kartid.append(all_receipt[i][3])
    session['kartids']=lst_kartid
    lst_products=[]
    lst_amounts=[]
    for i in range(len(all_receipt)):
...
def complete():
    ...
    if len(lst_sanpham)==0:
        ...
        return redirect(url_for('cart'))
    new_com=Complete(user_name=current_user.name,product_name=lst_sanpham,total_money=session['total_money'],time=session['time'],address=add,payment=payment)
    db.session.add(new_com)
    db.session.commit()
    for kartid in session['kartids']:
        Kart.query.filter_by(kartid=kartid).delete()
        db.session.commit()
    if payment=="Credit":
        flash("Your ID Cart is "+str(new_com.comid)+". We will send you products as soon as possible when receive money.")
...
def raw_materials():
    ...
    check=Count_Visit.query.filter_by(date_visit=time).first()
    if check is None:
        new_count=Count_Visit(date_visit=time,web_home=0,web_about=0,web_store=0,web_booking=0,web_coffeesraw=0,web_machinery=1)
        db.session.add(new_count)
        db.session.commit()
    else:
...
def booking():
    ...
    check=Count_Visit.query.filter_by(date_visit=time).first()
    if check is None:
        new_count=Count_Visit(date_visit=time,web_home=0,web_about=0,web_store=0,web_booking=1,web_coffeesraw=0)
        db.session.add(new_count)
        db.session.commit()
    else:
...
def store():
    ...
    check=Count_Visit.query.filter_by(date_visit=time).first()
    if check is None:
        new_count=Count_Visit(date_visit=time,web_home=0,web_about=0,web_store=1,web_booking=0,web_coffeesraw=0)
        db.session.add(new_count)
        db.session.commit()
    else:
...
@app.route('/alldeal')
def alldeal():
    lst_deal=Complete.query.filter_by(user_name=current_user.name).with_entities(Complete.comid,Complete.user_name,Complete.product_name,Complete.total_money,Complete.time,Complete.address,Complete.payment).all()
    ...
    if request.method =='POST':
        ...
        if request.files:
            ...
            if file.filename == '' and len(phone)>0 :
                edituser=User.query.filter_by(id=current_user.get_id()).first()
            ...
            if file and allowed_file(file.filename):
                ...
                edituser.link_img=url_for('static',filename=new_filename)
                if len(phone)>0:
                    edituser.phone=phone
                db.session.commit()
                return redirect(url_for('myaccount',user=current_user))
...
def signup():
    if request.method=='POST': 
        ...
        if not phone.isnumeric():
            ...
            return redirect(url_for('signup'))
        msg = Message('Hello', sender = 'thuchanhtin20173005@gmail.com', recipients = [email])
        code_confirm=np.random.randint(1000,9999)
        session['code_confirm']=str(code_confirm)
        msg.body = "Hello Flask message sent from Flask-Mail. Your code is "+ str(code_confirm)
        mail.send(msg)
...
def confirm():
    if request.method=='POST':
        if request.form['yourcode']==session['code_confirm']:
            ...
            new_user = User(email=email, name=name, password=generate_password_hash(password, method='sha256'),link_img=url_for('static', filename='img/bg.jpg'),phone=phone)
