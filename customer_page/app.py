...
def complete():
    ...
    if len(lst_sanpham)==0:
        ...
        return redirect(url_for('cart'))
    new_com=Complete(userID=current_user.get_id(),product_name=lst_sanpham,total_money=session['total_money'],time=session['time'],address=add,payment=payment)
...
@app.route('/alldeal')
def alldeal():
    lst_deal=Complete.query.filter_by(userID=current_user.get_id()).with_entities(Complete.comid,Complete.userID,Complete.product_name,Complete.total_money,Complete.time,Complete.address,Complete.payment).all()
